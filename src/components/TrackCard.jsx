    ...
    <div className={`flex items-center justify-between p-3 rounded hover:bg-neutral-800 transition group ${isFavorite ? 'bg-neutral-800 border border-green-900/30' : 'bg-neutral-900'}`}>
      ...
      <div className="flex items-center gap-3 overflow-hidden flex-1">
        <img 
          ...
          className="w-12 h-12 rounded object-cover cursor-pointer hover:opacity-80 transition"
        />
        
        <div className="flex flex-col truncate pr-2">
          <span 
            className={`font-semibold truncate cursor-pointer hover:underline ${isFavorite ? 'text-green-400' : 'text-white'}`}
            onClick={() => onOpenModal(track)}
          >
            {track.name}
          </span>
